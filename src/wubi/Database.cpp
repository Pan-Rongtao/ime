...
/////////////////
bool Query::search(const std::string & zigen, Query::Condition zigenCondition)
{
	release();
	if (zigen.empty())
		return true;

	std::string sQueryZigen = zigenCondition == Condition::like ? std::string("like '") + zigen + "%'" : std::string("= '") + zigen + "'";
	std::string sCmd = "select distinct word from (select word, priority from system where zigen " + sQueryZigen + ") order by priority desc";
	char *error = nullptr;
...
{
	int ret = 0;
	std::string sCmd = "select priority from system where zigen like '" + firstLetter + "%' order by priority desc";
	char *error = nullptr;
	char **data = nullptr;
	int row = 0, col = 0;
