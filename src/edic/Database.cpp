...
{
	char *error = nullptr;
	std::string sCmdSystemTab = "delete from system where word = '" + word + "'";
	std::string sCmdLearnTab = "delete from learn where word = '" + word + "'";
...
{
	std::string sCmdSystemTab = "update system set priority = " + std::to_string(proiority) + " where word = '" + word + "'";
	std::string sCmdLearnTab = "update learn set priority = " + std::to_string(proiority) + " where word = '" + word + "'";
	char *error = nullptr;
	return sqlite3_exec(m_sqlHandle, sCmdSystemTab.data(), nullptr, nullptr, &error) == SQLITE_OK
		&& sqlite3_exec(m_sqlHandle, sCmdLearnTab.data(), nullptr, nullptr, &error) == SQLITE_OK;
}
...
bool Query::search(const std::string &word, Condition wordCondition)
{
	...
	std::string sQueryWord = wordCondition == Condition::like ? std::string("like '") + word + "%'" : std::string("= '") + word + "'";	//不使用"_%'"能搜索到自己，比如love 可以搜索到love
	std::string sCmd = "select distinct word, priority from (select word, priority from system where word " + sQueryWord +
		" union select word, priority from learn where word " + sQueryWord + ") order by priority desc";
	char *error = nullptr;
